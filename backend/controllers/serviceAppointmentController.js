...
export const confirmServicePayment = async (req,res) => {
  try {
    ...
    try{session = await stripe.checkout.sessions.retrieve(session_id);}
     ...
     console.error("Stripe session not found", err);
...
// to  cancel the service Appointments
...
export const getServiceAppointmentStats = async (req,res) => {
  try {
    ...
    console.error("getServiceAppointmentStatserror:", error);
    return res.status(500).json({
      success:false,
      message:"Server error"
    });
...
//to get appointment for patient
...
export const getServiceAppointmentByPatient = async (req,res) =>{
  try {
    ...
    if(!resolveClerkUserId && !mobile) return res.json({
      success:true,
      date: []
    });
    const filter = {} ;
    ...
    console.error("getServiceAppointmentByPatient:", error);
    return res.status(500).json({
      success:false,
      message:"Server error"
    });
