...
import {
  uploadToCloudinary,
  deleterFromCloudinary,
  
} from "../utils/cloudinary.js";

//helper function

//this function converts array like input into a clean array
...
//this function takes date-time slot strings and group
function normalizeSlotsToMap(slotStrings = []) {
  ...
  slotStrings.forEach((raw) => {
    ...
    const timeStr = `${String(Number(hour)).padStart(2, "0")}:${String(
      minute
    ).padStart(2, "0")} ${ampm.toUpperCase()}`;
...
export async function deleteService(req, res) {
  try {
    ...
    if (existing.imagePublicId) {
      try {
        await deleterFromCloudinary(existing.imagePublicId);
      } catch (err) {
        console.warn(
