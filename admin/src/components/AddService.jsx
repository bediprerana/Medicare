...
import { AlertTriangle, CheckCircle, Clock, XCircle, Image } from 'lucide-react';
...
const AddService = ({serviceId}) => {
  ...
  // to fetch services when in the editing stage
 useEffect(() => {
    let mounted = true;
...
: addServiceStyles.toast.toastSuccess
        ...
        <div className="flex-1 min-w-0">
          <div className={addServiceStyles.toast.title}>{toast.title}
            </div>
            <div className={addServiceStyles.toast.message}>{toast.message}</div>

        </div>
        <button onClickCapture={() => setToast(null)}
