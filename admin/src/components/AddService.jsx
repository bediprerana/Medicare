...
const AddService = ({ serviceId }) => {
  ...
  const daysInSelectedMonth = new Date(selectedYearNum, selectedMonthNum + 1, 0).getDate();
  ...
  }, [slotDay, slotMonth, slotYear, daysInSelectedMonth]);
  ...
  useEffect(() => {
    ...
    async function loadService() {
      ...
      try {
        ...
        setInstructions(Array.isArray(data.instructions) && data.instructions.length ? data.instructions : [""]);
...
function removeInstruction(index) {
  ...
  function addSlot() {
    const formatted = `${String(slotDay).padStart(2, "0")} ${months[Number(slotMonth)]} ${slotYear} • ${String(slotHour).padStart(2, "0")}:${slotMinute} ${slotAmPm}`;

    if (slots.includes(formatted)) {
      showToast("error", "Duplicate Slot", "This time slot has already been added.");
      return;
    }

    if (selectedDateTime().getTime() <= Date.now()) {
      setErrors((prev) => ({ ...prev, slots: true }));
      showToast("error", "Past Time", "Please select a future date/time.");
      return;
    }
  ...
  function validate() {
    ...
    if (!instructions.some((item) => item.trim())) newErrors.instructions = true;
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      fd.append("price", String(price).replace(/[^\d.-]/g, "") || "0");
      ...
      const url = serviceId ? `${API_BASE}/api/services/${serviceId}` : `${API_BASE}/api/services`;
      ...
      if (!res.ok) {
        showToast("error", "Save Failed", data?.message || `Server error (${res.status})`);
        return;
      }

      showToast("success", serviceId ? "Service Updated" : "Service Added", `${serviceName} saved successfully.`);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
          ...
          <div className={addServiceStyles.headerActions}>
            <button
              type="button"
              onClick={resetForm}
              className={addServiceStyles.buttons?.reset || "px-4 py-2 rounded-lg bg-gray-200"}
            >
              Reset
            </button>
            ...
            >
              {submitting ? "Saving..." : serviceId ? "Update Service" : "Save Service"}
            </button>
          </div>
        </div>
      ...
      <div>
        ...
        <div className={addServiceStyles.grids.slotsGrid}>
          {slots.length === 0 ? (
            ...
            slots.map((slot, idx) => (
              <div key={slot} className={addServiceStyles.slots.slotItem}>
